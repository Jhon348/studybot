...
async def send_question(update, ctx):
    ...
    try:
        ...
        await ctx.bot.send_message(chat_id=chat_id, text="❌ Error. Escribe /nuevo para reiniciar.")
    return IN_QUIZ
...
def main():
    ...
    # Global fallback callbacks — capturan botones cuando el estado de conversación se pierde
    app.add_handler(CallbackQueryHandler(menu_callback, pattern="^(mode_|show_progress|back_menu)"))
    app.add_handler(CallbackQueryHandler(exam_length_callback, pattern="^exam_"))
    app.add_handler(CallbackQueryHandler(chapter_callback, pattern="^chapter_"))
    app.add_handler(CallbackQueryHandler(handle_answer_callback, pattern="^ans_"))
    app.add_handler(CallbackQueryHandler(next_question_callback, pattern="^next_question$"))
    app.add_handler(CallbackQueryHandler(explain_current_callback, pattern="^explain_current$"))
