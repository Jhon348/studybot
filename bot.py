...
async def check_access(update: Update) -> bool:
    """Devuelve True si el usuario tiene acceso, False si no y envía mensaje."""
...
async def cmd_activar(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    args = ctx.args
    if not args:
        await update.message.reply_text("Uso: /activar <user_id> <nombre opcional>")
        return
    uid = args[0]
    ...
    await update.message.reply_text(f"✅ Usuario *{nombre}* (`{uid}`) activado.", parse_mode="Markdown")
...
async def cmd_clientes(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, info in clients.items():
        estado = "✅" if info.get("active") else "🚫"
        lines.append(f"{estado} {info.get('name', 'Sin nombre')} — `{uid}`")
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "*Comandos:*\n/simulacro — Examen de 20, 50 o 100 preguntas\n"
        "/resumen — Resumen de capítulo\n/explicar — Explicar respuesta\n"
        "/progreso — Ver estadísticas\n/repasar — Solo tus fallos\n\n"
        "📎 *Envíame un PDF para empezar.*", parse_mode="Markdown")
    return WAITING_PDF


async def handle_pdf(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    if not await check_access(update):
        return ConversationHandler.END
    user_id = update.effective_user.id
    ...
    try:
        ...
        for i, chapter in enumerate(chapters):
            chapter["_idx"] = i
            await msg.edit_text(f"🤖 Generando preguntas... capítulo {i+1}/{len(chapters)}")
        ...
        await msg.edit_text(
            ...
            f"📂 Capítulos:\n{chapter_list}\n\n❓ *{len(all_questions)} preguntas generadas*\n\n¿Qué quieres hacer?",
...
async def menu_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_id = update.effective_user.id
    if data == "mode_quick":
        queue = spaced_rep.get_study_queue(user_id, ctx.user_data.get("questions", []), limit=10)
        ctx.user_data.update({"quiz_queue": queue, "quiz_index": 0, "quiz_correct": 0, "quiz_total": len(queue)})
        await query.edit_message_text("🎯 *Práctica rápida — 10 preguntas*", parse_mode="Markdown")
        return await send_question(update, ctx)
    elif data == "mode_exam":
        await query.edit_message_text("📝 ¿Cuántas preguntas?", reply_markup=InlineKeyboardMarkup([
        ...
        if not failed:
            await query.edit_message_text("🎉 ¡No tienes fallos todavía!", reply_markup=main_menu_keyboard())
            return CHOOSING_MODE
        ctx.user_data.update({"quiz_queue": failed[:20], "quiz_index": 0, "quiz_correct": 0, "quiz_total": len(failed[:20])})
        await query.edit_message_text(f"🔁 *Repaso — {len(failed[:20])} preguntas*", parse_mode="Markdown")
        return await send_question(update, ctx)
    elif data == "mode_chapter":
        chapters = ctx.user_data.get("chapters", [])
        keyboard = [[InlineKeyboardButton(f"📖 {ch['title'][:40]}", callback_data=f"chapter_{i}")] for i, ch in enumerate(chapters)]
        ...
        await query.edit_message_text(format_progress(stats), parse_mode="Markdown",
...
async def exam_length_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    n = {"exam_20": 20, "exam_50": 50, "exam_100": 100}.get(query.data, 20)
    queue = spaced_rep.get_study_queue(update.effective_user.id, ctx.user_data.get("questions", []), limit=n)
    ctx.user_data.update({"quiz_queue": queue, "quiz_index": 0, "quiz_correct": 0, "quiz_total": len(queue)})
    await query.edit_message_text(f"📝 *Simulacro de {len(queue)} preguntas*", parse_mode="Markdown")
    return await send_question(update, ctx)


async def chapter_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    idx = int(query.data.split("_")[1])
    ch_qs = [q for q in ctx.user_data.get("questions", []) if q.get("chapter_idx") == idx][:20]
    ctx.user_data.update({"quiz_queue": ch_qs, "quiz_index": 0, "quiz_correct": 0, "quiz_total": len(ch_qs)})
    await query.edit_message_text(f"📖 *{ctx.user_data['chapters'][idx]['title']}*\n{len(ch_qs)} preguntas", parse_mode="Markdown")
    return await send_question(update, ctx)


async def send_question(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if question["type"] == "multiple_choice":
        ...
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(opt, callback_data=f"ans_{opt}") for opt in question["options"]]])
    elif question["type"] == "true_false":
        text = header + f"🔵 *¿Verdadero o Falso?*\n\n_{question['question']}_"
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Verdadero", callback_data="ans_Verdadero"), InlineKeyboardButton("❌ Falso", callback_data="ans_Falso")]])
    else:
...
async def handle_answer_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    user_id = update.effective_user.id
    ...
    await query.edit_message_text(result_text, parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("➡️ Siguiente", callback_data="next_question"), InlineKeyboardButton("💬 Explicar", callback_data="explain_current")]]))
    return IN_QUIZ


async def handle_open_answer(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    evaluation = await question_gen.evaluate_open_answer(question["question"], question["answer"], update.message.text)
    ...
    await msg.edit_text(
        ...
        parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("➡️ Siguiente", callback_data="next_question")]]))
    return IN_QUIZ


async def next_question_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    await update.callback_query.answer()
    return await send_question(update, ctx)


async def explain_current_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    question = ctx.user_data.get("current_q")
    if not question:
    ...
    await query.edit_message_text(f"💡 *Explicación*\n\n{explanation}",
        parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("➡️ Siguiente", callback_data="next_question")]]))
    return IN_QUIZ


async def show_quiz_results(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔁 Repasar fallos", callback_data="mode_review"), InlineKeyboardButton("🏠 Menú", callback_data="back_menu")]])
    send = update.callback_query.edit_message_text if update.callback_query else update.effective_message.reply_text
    await send(text, parse_mode="Markdown", reply_markup=keyboard)
    return CHOOSING_MODE


async def cmd_explicar(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await check_access(update):
        return
    question = ctx.user_data.get("current_q")
    if not question:
...
async def cmd_resumen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not chapters:
        ...
        return
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"📖 {ch['title'][:40]}", callback_data=f"summary_{i}")] for i, ch in enumerate(chapters)])
    await update.message.reply_text("¿De qué capítulo?", reply_markup=keyboard)


async def summary_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    idx = int(query.data.split("_")[1])
    ...
    await query.edit_message_text(f"📖 *{chapter['title']}*\n\n{summary}", parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data="back_menu")]]))
    return CHOOSING_MODE
...
async def cmd_repasar(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    if not await check_access(update):
        return ConversationHandler.END
    user_id = update.effective_user.id
    failed = progress_tracker.get_failed_questions(user_id, ctx.user_data.get("questions", []))
...
async def cmd_nuevo(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    if not await check_access(update):
        return ConversationHandler.END
    ctx.user_data.clear()
...
async def cmd_simulacro(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    if not await check_access(update):
        return ConversationHandler.END
    if not ctx.user_data.get("questions"):
        await update.message.reply_text("📎 Primero sube un PDF.")
        return ConversationHandler.END
    await update.message.reply_text("📝 ¿Cuántas preguntas?", reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("20 preguntas", callback_data="exam_20")],
        [InlineKeyboardButton("50 preguntas", callback_data="exam_50")],
        [InlineKeyboardButton("100 preguntas", callback_data="exam_100")],
    ]))
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[CommandHandler("start", start), CommandHandler("nuevo", cmd_nuevo)],
        states={
            ...
            CHOOSING_EXAM_LEN: [CallbackQueryHandler(exam_length_callback, pattern="^exam_")],
            CHOOSING_CHAPTER: [CallbackQueryHandler(chapter_callback, pattern="^chapter_")],
            IN_QUIZ: [
        ...
        fallbacks=[
            CommandHandler("start", start), CommandHandler("nuevo", cmd_nuevo),
            CommandHandler("progreso", cmd_progreso), CommandHandler("resumen", cmd_resumen),
            CommandHandler("simulacro", cmd_simulacro), CommandHandler("repasar", cmd_repasar),
            CommandHandler("explicar", cmd_explicar),
        ],
        allow_reentry=True,
    )
