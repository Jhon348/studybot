...
async def send_question(ctx, chat_id, user_data):
...
async def cmd_start(update, ctx):
    ...
    await update.message.reply_text(
        ...
        "*Comandos:*\n/simulacro /resumen /explicar /progreso /repasar /nuevo",
        parse_mode="Markdown")
...
async def cmd_repasar(update, ctx):
    ...
    await send_question(ctx, update.effective_chat.id, ctx.user_data)
...
async def handle_callback(update, ctx):
    ...
    if data == "mode_quick":
        ...
        await send_question(ctx, chat_id, ctx.user_data)
