...
from database import (
    db_get_client, db_upsert_client, db_set_active, db_increment_trial,
    db_get_all_clients, db_record_answer, db_get_stats, db_get_failed_questions,
    db_get_expiring_clients, db_mark_notified, db_deactivate_expired
)
...
def is_active(user_id):
    if user_id == ADMIN_ID:
        return True
    client = db_get_client(user_id)
    return client is not None and client.get("active", False)
...
def get_trial_questions_used(user_id):
    ...
    if client is None:
        return 0
    return client.get("trial_questions", 0)


def increment_trial_questions(user_id):
    db_increment_trial(user_id)
...
async def cmd_progreso(update, ctx):
    if not await check_access(update):
        return
    stats = db_get_stats(update.effective_user.id)
...
async def cmd_repasar(update, ctx):
    if not await check_access(update):
        return
    user_id = update.effective_user.id
    failed = db_get_failed_questions(user_id, ctx.user_data.get("questions", []))
    if not failed:
...
async def cmd_simulacro(update, ctx):
    ...
    for n in [20, 50, 100]:
        if total_q >= n:
            buttons.append([InlineKeyboardButton(f"{n} preguntas", callback_data=f"exam_{n}")])
    if total_q not in [20, 50, 100]:
        buttons.append([InlineKeyboardButton(f"{total_q} preguntas — todas", callback_data=f"exam_{total_q}")])
    buttons.append([InlineKeyboardButton("🔄 Generar más preguntas", callback_data="gen_more")])
    await update.message.reply_text(
        f"📝 *Simulacro de examen*\n\n"
        f"📊 Tienes *{total_q} preguntas* disponibles\n\n"
        f"¿Cuántas quieres?",
        parse_mode="Markdown",
...
async def cmd_activar(update, ctx):
    ...
    args = ctx.args
    if not args:
        await update.message.reply_text("Uso: /activar <user_id> <nombre> <días>\nEjemplo: /activar 123456 Juan 30")
        return
    uid = int(args[0])
    # Detectar si el último arg es número de días
    days = 30
    name_parts = args[1:]
    if name_parts and name_parts[-1].isdigit():
        days = int(name_parts[-1])
        name_parts = name_parts[:-1]
    nombre = " ".join(name_parts) if name_parts else "Cliente"
    db_set_active(uid, True, nombre, days)
    from datetime import datetime, timedelta
    expires = (datetime.utcnow() + timedelta(days=days)).strftime("%d/%m/%Y")
    await update.message.reply_text(
        f"✅ *{nombre}* activado\n"
        f"📅 Vence el: *{expires}* ({days} días)\n"
        f"🆔 ID: `{uid}`",
        parse_mode="Markdown")
            ...
            if expires_at:
                try:
                    ...
                    if days_left < 0:
                        ...
                        estado = f"⚠️ Vence en {days_left} días"
                    else:
                        estado = f"✅ Activo — {days_left} días restantes"
                except Exception:
                    estado = "✅ Activo"
            else:
                estado = "✅ Activo"
        else:
            used = c.get("trial_questions", 0)
            estado = f"🆓 Prueba ({used}/{FREE_QUESTIONS} preguntas)"
        lines.append(f"{estado}\n  └ {c.get('name','?')} — `{c['user_id']}`")
    await update.message.reply_text(
...
async def handle_pdf(update, ctx):
    ...
    # Verificar tamaño del archivo (máximo 18 MB)
    file_size = update.message.document.file_size or 0
    max_size = 18 * 1024 * 1024  # 18 MB
    if file_size > max_size:
        size_mb = round(file_size / 1024 / 1024, 1)
        await update.message.reply_text(
            f"⚠️ Tu PDF pesa *{size_mb} MB* y es demasiado grande.\n\n"
            "📎 El límite es *18 MB*. Por favor:\n"
            "• Divide el PDF en partes más pequeñas\n"
            "• O comprime el PDF en smallpdf.com (gratis)",
            parse_mode="Markdown"
        )
        return
    ...
    try:
        ...
        if not chapters:
            ...
            return
        ctx.user_data.clear()
        # Limitar a 10 capítulos máximo para evitar timeout
        if len(chapters) > 10:
            await msg.edit_text(
                f"📖 Se detectaron {len(chapters)} secciones. "
                f"Procesando las primeras 10 para mayor velocidad..."
            )
...
async def handle_callback(update, ctx):
    ...
    if data == "mode_quick":
        ...
        # Construir botones dinámicos según preguntas disponibles
        buttons = []
        for n in [20, 50, 100]:
            if total_q >= n:
                buttons.append([InlineKeyboardButton(f"{n} preguntas", callback_data=f"exam_{n}")])
        # Siempre agregar opción de máximo disponible si no es múltiplo exacto
        if total_q not in [20, 50, 100]:
            buttons.append([InlineKeyboardButton(f"{total_q} preguntas — todas las disponibles", callback_data=f"exam_{total_q}")])
        buttons.append([InlineKeyboardButton("🔄 Generar más preguntas", callback_data="gen_more")])
        ...
        n = int(data.split("_")[1]) if data.split("_")[1].isdigit() else 20
        questions = ctx.user_data.get("questions", [])
        if not questions:
            await query.message.reply_text("📎 Primero sube un PDF con /nuevo")
            return
        ...
        questions = ctx.user_data.get("questions", [])
        failed = db_get_failed_questions(user_id, questions)
        if not failed:
        ...
        db_record_answer(user_id, question["id"], is_ok)
...
def main():
    ...
    # Job de verificación de vencimientos cada hora
    async def check_expiry(context):
        expiring = db_get_expiring_clients()
        for c in expiring:
            try:
                await context.bot.send_message(
                    chat_id=c["user_id"],
                    text=(
                        "⚠️ *Tu suscripción de StudyBot vence hoy*\n\n"
                        "Para continuar estudiando sin interrupciones, renueva tu acceso.\n\n"
                        f"📲 Contáctame: @{ADMIN_USERNAME}"
                    ),
                    parse_mode="Markdown"
                )
                db_mark_notified(c["user_id"])
            except Exception as e:
                logger.error(f"Error notificando vencimiento a {c['user_id']}: {e}")
        # Desactivar los vencidos
        n = db_deactivate_expired()
        if n:
            logger.info(f"Desactivados {n} clientes vencidos")
